...
def main():
    ...
    save_dir = './result'
    pd.DataFrame(X_new.cpu().numpy()).to_csv(f"{save_dir}/{timestamp}_cold_X.csv", index=False)
    pd.DataFrame(Y_new.cpu().numpy()).to_csv(f"{save_dir}/{timestamp}_cold_Y.csv", index=False)
