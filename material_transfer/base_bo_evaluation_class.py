...
class BaseBOEvaluation(BaseBO):
    def __init__(self, input_dim, objective_dim, seed=None, device=None):
        ...
        self.hyper_volume_log = torch.empty((0, 1), device=self.device)
        self.gd_mean_log = torch.empty((0, 1), device=self.device)
        self.igd_mean_log = torch.empty((0, 1), device=self.device)
        self.spacing_mean_log = torch.empty((0, 1), device=self.device)
        self.cardinality_mean_log = torch.empty((0, 1), device=self.device)
